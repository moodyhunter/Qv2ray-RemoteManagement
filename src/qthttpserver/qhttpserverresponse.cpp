...
QHttpServerResponsePrivate::QHttpServerResponsePrivate(QByteArray &&d, const QHttpServerResponse::StatusCode sc) : data(std::move(d)), statusCode(sc)
{
}

QHttpServerResponsePrivate::QHttpServerResponsePrivate(const QHttpServerResponse::StatusCode sc) : statusCode(sc)
{
}

QHttpServerResponse::QHttpServerResponse(QHttpServerResponse &&other) noexcept : d_ptr(other.d_ptr.take())
{
}
...
QHttpServerResponse::QHttpServerResponse(const char *data) : QHttpServerResponse(QByteArray::fromRawData(data, qstrlen(data)))
{
}

QHttpServerResponse::QHttpServerResponse(const QString &data) : QHttpServerResponse(data.toUtf8())
{
}

QHttpServerResponse::QHttpServerResponse(const QByteArray &data) : QHttpServerResponse(QMimeDatabase().mimeTypeForData(data).name().toLocal8Bit(), data)
{
}

QHttpServerResponse::QHttpServerResponse(QByteArray &&data) : QHttpServerResponse(QMimeDatabase().mimeTypeForData(data).name().toLocal8Bit(), std::move(data))
{
}

QHttpServerResponse::QHttpServerResponse(const QJsonObject &data)
    : QHttpServerResponse(QHttpServerLiterals::contentTypeJson(), QJsonDocument(data).toJson(QJsonDocument::Compact))
{
}
...
QHttpServerResponse::QHttpServerResponse(const QByteArray &mimeType, const QByteArray &data, const StatusCode status)
    : d_ptr(new QHttpServerResponsePrivate(QByteArray(data), status))
{
    setHeader(QHttpServerLiterals::contentTypeHeader(), mimeType);
}

QHttpServerResponse::QHttpServerResponse(QByteArray &&mimeType, const QByteArray &data, const StatusCode status)
    : d_ptr(new QHttpServerResponsePrivate(QByteArray(data), status))
{
    setHeader(QHttpServerLiterals::contentTypeHeader(), std::move(mimeType));
}

QHttpServerResponse::QHttpServerResponse(const QByteArray &mimeType, QByteArray &&data, const StatusCode status)
    : d_ptr(new QHttpServerResponsePrivate(std::move(data), status))
{
    setHeader(QHttpServerLiterals::contentTypeHeader(), mimeType);
}

QHttpServerResponse::QHttpServerResponse(QByteArray &&mimeType, QByteArray &&data, const StatusCode status)
    : d_ptr(new QHttpServerResponsePrivate(std::move(data), status))
{
    setHeader(QHttpServerLiterals::contentTypeHeader(), std::move(mimeType));
}
...
QHttpServerResponse::QHttpServerResponse(QHttpServerResponsePrivate *d) : d_ptr(d)
{
    d->derived = true;
}
...
bool QHttpServerResponse::hasHeader(const QByteArray &name, const QByteArray &value) const
{
    ...
    auto condition = [&value](const std::pair<QByteArray, QByteArray> &pair) { return pair.second == value; };
...
void QHttpServerResponse::write(QHttpServerResponder &&responder) const
{
    ...
    responder.writeHeader(QHttpServerLiterals::contentLengthHeader(), QByteArray::number(d->data.size()));

...
/*! \fn template<typename ViewHandler, typename ViewTraits = QHttpServerRouterViewTraits<ViewHandler>> auto bindCaptured(ViewHandler &&handler, QRegularExpressionMatch
   &match) const -> typename ViewTraits::BindableType
...
QHttpServerRouterPrivate::QHttpServerRouterPrivate() : converters(defaultConverters)
{
}
...
*/
QHttpServerRouter::QHttpServerRouter() : d_ptr(new QHttpServerRouterPrivate)
{
}
...
bool QHttpServerRouter::addRuleImpl(QHttpServerRouterRule *rule, const std::initializer_list<int> &types)
{
    Q_D(QHttpServerRouter);
...
/*!
    ...
    matches the request.
*/
bool QHttpServerRouter::handleRequest(const QHttpServerRequest &request, QTcpSocket *socket) const
{
    Q_D(const QHttpServerRouter);

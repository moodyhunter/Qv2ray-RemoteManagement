...
*/
QHttpServerRouterRule::QHttpServerRouterRule(const QString &pathPattern, const QHttpServerRequest::Methods methods, RouterHandler &&routerHandler)
    : QHttpServerRouterRule(new QHttpServerRouterRulePrivate{ pathPattern, methods, std::forward<RouterHandler>(routerHandler), {} })
{
}
...
*/
QHttpServerRouterRule::QHttpServerRouterRule(const QString &pathPattern, const char *methods, RouterHandler &&routerHandler)
    : QHttpServerRouterRule(pathPattern, strToMethods(methods), std::forward<RouterHandler>(routerHandler))
{
}
...
/*!
    \internal
 */
QHttpServerRouterRule::QHttpServerRouterRule(QHttpServerRouterRulePrivate *d) : d_ptr(d)
{
}
...
*/
bool QHttpServerRouterRule::exec(const QHttpServerRequest &request, QTcpSocket *socket) const
{
    Q_D(const QHttpServerRouterRule);
...
/*!
    \internal
*/
bool QHttpServerRouterRule::createPathRegexp(const std::initializer_list<int> &metaTypes, const QMap<int, QLatin1String> &converters)
{
    ...
    {
        if (type >= QMetaType::User && !QMetaType::hasRegisteredConverterFunction((QMetaType) qMetaTypeId<QString>(), (QMetaType) type))
        {
            qCWarning(lcRouterRule) << QMetaType(type).name() << "has not registered a converter to QString."
        ...
        {
            qCWarning(lcRouterRule) << "Can not find converter for type:" << QMetaType(type).name();
            return false;
        }
    ...
    {
        qCWarning(lcRouterRule) << "not enough types or one of the types is not supported, regexp:" << pathRegexp << ", pattern:" << d->pathPattern

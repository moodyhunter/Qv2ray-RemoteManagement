...
QDebug operator<<(QDebug debug, const http_parser *const httpParser)
{
    ...
    debug << "HTTP " << httpParser->http_major << "." << httpParser->http_minor << " " << http_method_str(http_method(httpParser->method)) << ')';
    debug.setAutoInsertSpaces(oldSetting);
    return debug.maybeSpace();
}
#endif
...
static const std::array<void (*)(const QString &, QUrl *), UF_MAX> parseUrlFunctions{
    [](const QString &string, QUrl *url) { url->setScheme(string); },       [](const QString &string, QUrl *url) { url->setHost(string); },
    [](const QString &string, QUrl *url) { url->setPort(string.toInt()); }, [](const QString &string, QUrl *url) { url->setPath(string, QUrl::TolerantMode); },
    [](const QString &string, QUrl *url) { url->setQuery(string); },        [](const QString &string, QUrl *url) { url->setFragment(string); },
    [](const QString &string, QUrl *url) { url->setUserInfo(string); },
};

http_parser_settings QHttpServerRequestPrivate::httpParserSettings{ &QHttpServerRequestPrivate::onMessageBegin, &QHttpServerRequestPrivate::onUrl,
                                                                    &QHttpServerRequestPrivate::onStatus,       &QHttpServerRequestPrivate::onHeaderField,
                                                                    &QHttpServerRequestPrivate::onHeaderValue,  &QHttpServerRequestPrivate::onHeadersComplete,
                                                                    &QHttpServerRequestPrivate::onBody,         &QHttpServerRequestPrivate::onMessageComplete,
                                                                    &QHttpServerRequestPrivate::onChunkHeader,  &QHttpServerRequestPrivate::onChunkComplete };

QHttpServerRequestPrivate::QHttpServerRequestPrivate(const QHostAddress &remoteAddress) : remoteAddress(remoteAddress)
{
...
bool QHttpServerRequestPrivate::parse(QIODevice *socket)
{
    ...
    if (fragment.size())
    {
        ...
        url.setScheme(sslSocket && sslSocket->isEncrypted() ? QStringLiteral("https") : QStringLiteral("http"));
#else
        url.setScheme(QStringLiteral("http"));
#endif
        const auto parsed = http_parser_execute(&httpParser, &httpParserSettings, fragment.constData(), size_t(fragment.size()));
...
bool QHttpServerRequestPrivate::parseUrl(const char *at, size_t length, bool connect, QUrl *url)
{
    ...
    for (auto i = 0u; i < UF_MAX; i++)
    {
        if (u.field_set & (1 << i))
        {
            parseUrlFunctions[i](QString::fromUtf8(at + u.field_data[i].off, u.field_data[i].len), url);
        }
    }
    return true;
}
...
int QHttpServerRequestPrivate::onBody(http_parser *httpParser, const char *at, size_t length)
{
    ...
    if (i->body.isEmpty())
    {
        i->body.reserve(static_cast<int>(httpParser->content_length) + static_cast<int>(length));
    }
...
QHttpServerRequest::QHttpServerRequest(const QHostAddress &remoteAddress) : d(new QHttpServerRequestPrivate(remoteAddress))
{
}
...
QHttpServerRequest::Method QHttpServerRequest::method() const
{
    ...
    {
        case HTTP_GET: return QHttpServerRequest::Method::Get;
        case HTTP_PUT: return QHttpServerRequest::Method::Put;
        case HTTP_DELETE: return QHttpServerRequest::Method::Delete;
        case HTTP_POST: return QHttpServerRequest::Method::Post;
        case HTTP_HEAD: return QHttpServerRequest::Method::Head;
        case HTTP_OPTIONS: return QHttpServerRequest::Method::Options;
        case HTTP_PATCH: return QHttpServerRequest::Method::Patch;
        case HTTP_CONNECT: return QHttpServerRequest::Method::Connect;
        default: return QHttpServerRequest::Method::Unknown;
    }
}

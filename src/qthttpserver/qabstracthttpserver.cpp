...
void QAbstractHttpServerPrivate::handleNewConnections()
{
    ...
    while (auto socket = tcpServer->nextPendingConnection())
    {
        ...
        QObject::connect(socket, &QTcpSocket::readyRead, q_ptr, [this, request, socket]() { handleReadyRead(socket, request); });
        ...
        QObject::connect(socket, &QObject::destroyed, socket, [request]() { delete request; });
    }
}

void QAbstractHttpServerPrivate::handleReadyRead(QTcpSocket *socket, QHttpServerRequest *request)
{
    Q_Q(QAbstractHttpServer);
    Q_ASSERT(socket);
    Q_ASSERT(request);
    ...
    if (request->d->httpParser.upgrade && request->d->httpParser.method != HTTP_CONNECT)
    { // Upgrade
...
QAbstractHttpServer::QAbstractHttpServer(QObject *parent) : QAbstractHttpServer(*new QAbstractHttpServerPrivate, parent)
{
}

QAbstractHttpServer::QAbstractHttpServer(QAbstractHttpServerPrivate &dd, QObject *parent) : QObject(dd, parent)
{
#if defined(QT_WEBSOCKETS_LIB)
...
quint16 QAbstractHttpServer::listen(const QHostAddress &address, quint16 port)
{
#if QT_CONFIG(ssl)
    Q_D(QAbstractHttpServer);
    QTcpServer *tcpServer = d->sslEnabled ? new QSslServer(d->sslConfiguration, this) : new QTcpServer(this);
#else
    ...
    {
        qCCritical(lcHttpServer, "listen failed: %s", tcpServer->errorString().toStdString().c_str());
    }
...
QVector<quint16> QAbstractHttpServer::serverPorts()
{
    ...
    std::transform(children.cbegin(), children.cend(), std::back_inserter(ports), [](const QTcpServer *server) { return server->serverPort(); });
    return ports;
}
...
void QAbstractHttpServer::bind(QTcpServer *server)
{
    ...
    if (!server)
    {
        ...
        if (!server->listen())
        {
            qCCritical(lcHttpServer, "QTcpServer listen failed (%s)", qPrintable(server->errorString()));
        }
    }
    else
    {
    ...
    QObjectPrivate::connect(server, &QTcpServer::newConnection, d, &QAbstractHttpServerPrivate::handleNewConnections, Qt::UniqueConnection);
}
...
QHttpServerResponder QAbstractHttpServer::makeResponder(const QHttpServerRequest &request, QTcpSocket *socket)
{
